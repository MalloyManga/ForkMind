...
- **中文注释标点规范**：新写入的中文注释避免使用中文标点，例如 `，`、`、`、`。`；优先使用空格或 ASCII 标点，方便终端与编码环境稳定显示。
- **可读性规范**：复杂函数除函数头注释外，还应对关键分支、关键变量（尤其布尔判断与状态流转）补充就地注释，帮助快速审查。
- **参数与变量命名规范**：函数入参与局部变量优先使用语义化命名；若抽象命名不可避免，需在就地注释中明确其业务含义与数据来源。
- **判别联合分支规范**：对 `ConversationNodeType` 或其他判别联合做分支时 优先使用 `switch + assertNever` 不要用二分三元或 `else` 隐式吞掉未来新增类型。
