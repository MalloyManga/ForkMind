...
- 📎 **文本锚点与扩展卡片**：
  - 可从编辑器或画布 Markdown 中选中词句创建追问，并保存精确引用范围与文本快照。
  - 支持图片、链接和文件卡片；链接仅保存元数据，不主动抓取远程内容。
- 🗂️ **本地托管资产**：
  - 图片与文件统一复制到工作区 `assets/`，通过 SHA-256 去重并校验类型、大小与路径安全。
  - 自动保存继续保持轻量；显式导出时才把引用资产嵌入单个 JSON，便于跨设备迁移。
  - AI 上下文只包含文件名、MIME、大小和用户描述，不包含二进制内容或本地路径。
...
- **本地数据层**:
  - 用户配置目录中的 `workspace.json + threads/*.json + assets/`，采用临时文件与 `.bak` 恢复策略。
  - 支持完整工作区单文件导入导出、资产完整性校验与 ForkMind JSON 系统剪贴板。
...
- [x] 阶段 8：Go 上下文组装算法（`parentId` 主链遍历 + `referenceNodeIds` 参考注入 + 文本锚点背景注入）。
...
- [x] 支持在卡片文本中选中词/句直接追问：从选中内容创建新卡片，并记录引用锚点。
- [x] 支持扩展卡片类型：图片卡片（Image Node）、链接卡片（Link Node）、文件卡片（File Node）等。
